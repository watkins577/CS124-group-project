...
public class JavaGenerator {
	...
	public static boolean createJavaCode(Class _class) {
		File file = new File(new StringBuilder().append(_class.getName()).append(".java").toString());
		File file1 = new File(new StringBuilder().append("tmp_").append(_class.getName()).append(".java").toString());
		try {
	...
	private static String getMethodStatementFromMethod(Method m) {
		...
		statement.append(" ").append(m.getReturns()).append(" ").append(m.getName()).append("(");
		int i = 0;
	...
	private static String getFieldStatementFromField(Field f) {
		...
		statement.append(" ").append(f.getType()).append(" ").append(f.getName()).append(";");
		return statement.toString();
	}
}
